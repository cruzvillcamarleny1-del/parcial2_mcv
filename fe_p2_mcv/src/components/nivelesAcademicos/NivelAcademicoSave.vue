...
<template>
    <div class="card flex justify-center">
        <Dialog v-model:visible="dialogVisible" :header="props.modoEdicion ? 'Editar Nivel Académico' : 'Crear Nivel Académico'" style="width: 30rem">
                    <div class="mb-4">
                        <label for="nombre" class="font-semibold block mb-1">Nombre</label>
                        <InputText id="nombre" v-model="nivelAcademico.nombre" class="w-full" autocomplete="off" autofocus />
                    </div>
                    <div class="mb-4">
                        <label for="descripcion" class="font-semibold block mb-1">Descripción</label>
                        <Textarea id="descripcion" v-model="nivelAcademico.descripcion" rows="3" class="w-full" autocomplete="off" />
                    </div>
